...
jobs:
  dbt-costgate:
    ...
    steps:
      ...
      - uses: Drichards124/dbt-costgate@v1.0.3
        with:
...
```bash
docker pull ghcr.io/drichards124/dbt-costgate:v1.0.3
docker run --rm -v "$PWD:/workspace" ghcr.io/drichards124/dbt-costgate:v1.0.3 check
```
