...
jobs:
  dbt-costgate:
    ...
    steps:
      ...
      - uses: Drichards124/dbt-costgate@v1.0.4
        with:
...
```bash
docker pull ghcr.io/drichards124/dbt-costgate:v1.0.4
docker run --rm -v "$PWD:/workspace" ghcr.io/drichards124/dbt-costgate:v1.0.4 check
```
